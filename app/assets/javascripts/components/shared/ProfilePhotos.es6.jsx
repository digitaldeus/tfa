class ProfilePhotos extends React.Component {
  ...
  render() {
    ...
    for(let x = 0; x < this.props.uploaders; x++){
      uploaders.push(
        <div
          className="small-6 medium-4 columns photos-uploader-column"
          key={x}>
          <ImageUploadModule
...
class ProfilePhotosGallery extends React.Component {
  ...
  render() {
    ...
    let items = photos.map((photo, index) => {
      ...
      return (
        <div className="small-4 columns photos-gallery-column"
          key={index}>
          <img src={src}
            width="100%"
            onClick={this.openGalleryAt.bind(this, index)}/>
          <i className="fa fa-times photos-remove"
    ...
    return (
      <div>
        ...
        <div className="row photos-gallery-row">
          {items}
        </div>
      </div>
    );
